...
@Component({
  ...
  template:`
            <p>Hello welcome to the add bar</p>
              ...
              <select>
                <option>-------</option>
                <option #Task_dependant (keyup)="0" *ngFor="let item of optList">{{item.Task_id}}</option>
              </select>
              <input #Task_schedule (keyup)="0"  name="Task_schedule" placeholder="Task_schedule"/>
              <button (click)="submitMe(Task_id.value,Task_des.value,Task_priority.value,Task_weight.value,Task_dependant.value,Task_schedule.value)">Save</button>
              `
})
